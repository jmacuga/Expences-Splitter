...
public:
  // two Trip constructors, one with file handle to file from which should it be
  // recreated
  Trip(std::string nm) : name(nm){};
  Trip(std::string nm, std::ifstream &myfile) : name(nm) {
    load_from_file(myfile);
  };
  void add_person(Person const &p); // adds  person
  void add_transaction(
      std::shared_ptr<Transaction>
          p_trans); // adds pointer to transaction, settles peoples balances
