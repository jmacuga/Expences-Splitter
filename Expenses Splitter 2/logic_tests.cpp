...
template <> void compare_to_test(float actual, float expected, int test_n) {
...
int main() {
  ...
  // test Person::add_to_balance()
  {
    ...
    compare_to_test<bool>(Transaction::fl_cmp(Milosz.get_balance(), 10.0), true,
                          3);
  }
  // test 2 Transactions::add_transaction()
  {
    // test case 1 - add collective transaction
    {
      ...
      std::shared_ptr<Transaction> trans =
          std::make_shared<CollectiveTransaction>(price, 1,
                                                  Person::Category::food);
      compare_to_test<size_t>(Transaction::fl_cmp(trans->get_money(), price),
                              true, 4);
      trip.add_transaction(trans);
      compare_to_test<bool>(
          Transaction::fl_cmp(trip.get_person(0).get_balance(), 2 * price / 3),
          true, 7);
      compare_to_test<bool>(
          Transaction::fl_cmp(trip.get_person(2).get_balance(), -price / 3),
          true, 8);
    }
    // test case 2 - add collective and specific transactions wi default person
    // categories
    {
      Trip trip("testtrip");
      Person Milosz(1, "Milosz");
      ...
      std::shared_ptr<Transaction> spectrans =
          std::make_shared<SpecificTransaction>(
              30.0f, 2, Person::Category::alcohol, receivers);
      compare_to_test<bool>(
          Transaction::fl_cmp(trip.get_person(0).get_balance(), 7.66667f), true,
          9);
      trip.add_transaction(spectrans);
      compare_to_test<bool>(
          Transaction::fl_cmp(trip.get_person(1).get_balance(), 16.166667f),
          true, 11);
      compare_to_test<bool>(
          Transaction::fl_cmp(trip.get_person(2).get_balance(), -13.833333f),
          true, 12);
      compare_to_test<bool>(
          Transaction::fl_cmp(trip.get_person(0).get_balance(), -2.33333f),
          true, 13);
    }
    ...
    // test case 4 - specify person category
    {
      // test person category compare
      {
        ...
        compare_to_test<bool>(p.category_compare(Person::Category::food), true,
                              16);
        compare_to_test<bool>(p.category_compare(Person::Category::alcohol),
                              true, 17);
      }
      Trip trip("testtrip");
      Person Milosz(1, "Milosz");
      ...
      std::shared_ptr<Transaction> trans =
          std::make_shared<CollectiveTransaction>(price, 1,
                                                  Person::Category::food);
      trip.add_transaction(trans);
      compare_to_test<bool>(Julia.category_compare(Person::Category::alcohol),
                            true, 18);
      compare_to_test<bool>(Julia.category_compare(Person::Category::food),
                            false, 19);
      compare_to_test<bool>(
          Transaction::fl_cmp(trip.get_person(0).get_balance(), 4.20000f), true,
          21);
      compare_to_test<bool>(
          Transaction::fl_cmp(trip.get_person(1).get_balance(), -4.20000f),
          true, 22);
      compare_to_test<bool>(
          Transaction::fl_cmp(trip.get_person(2).get_balance(), 0.0f), true,
          20);
...

    {
      ...
      trip.add_transaction(trans);
      compare_to_test<bool>(
          Transaction::fl_cmp(trip.get_person(0).get_balance(), 6), true, 24);
      compare_to_test<bool>(
          Transaction::fl_cmp(trip.get_person(1).get_balance(), -6), true, 25);
      trip.add_person(Julia);
      compare_to_test<bool>(
          Transaction::fl_cmp(trip.get_person(0).get_balance(), 6), true, 27);
      compare_to_test<bool>(
          Transaction::fl_cmp(trip.get_person(1).get_balance(), -6), true, 28);
    ...
    // transfers test1
    {
      ...
      trip.add_person(Jula);
      std::shared_ptr<Transaction> trans =
          std::make_shared<CollectiveTransaction>(30.0f, 1,
                                                  Person::Category::food);
      trip.add_transaction(trans);
    ...
    // transfers test2 - 4 people
    {
      ...
      std::shared_ptr<Transaction> trans1 =
          std::make_shared<CollectiveTransaction>(40.0f, 1,
                                                  Person::Category::food);
      std::shared_ptr<Transaction> trans2 =
          std::make_shared<CollectiveTransaction>(20.0f, 2,
                                                  Person::Category::nicotine);
      std::shared_ptr<Transaction> trans3 =
          std::make_shared<CollectiveTransaction>(80.0f, 1,
                                                  Person::Category::alcohol);
    ...
    // transfers test 3
    {
      ...
      std::shared_ptr<Transaction> trans1 =
          std::make_shared<CollectiveTransaction>(40.0f, 1,
                                                  Person::Category::food);
      std::shared_ptr<Transaction> trans2 =
          std::make_shared<CollectiveTransaction>(40.0f, 2,
                                                  Person::Category::nicotine);
      std::shared_ptr<Transaction> trans3 =
          std::make_shared<CollectiveTransaction>(80.0f, 1,
                                                  Person::Category::alcohol);
    ...
    // transfers test 4
    {
      ...
      std::shared_ptr<Transaction> trans1 =
          std::make_shared<CollectiveTransaction>(55.0f, 1,
                                                  Person::Category::food);
      std::shared_ptr<Transaction> trans2 =
          std::make_shared<CollectiveTransaction>(40.0f, 2,
                                                  Person::Category::nicotine);
      std::shared_ptr<Transaction> trans3 =
          std::make_shared<CollectiveTransaction>(30.0f, 3,
                                                  Person::Category::food);
      std::shared_ptr<Transaction> trans4 =
          std::make_shared<CollectiveTransaction>(100.0f, 4,
                                                  Person::Category::alcohol);
      trip.add_transaction(trans1);
      trip.add_transaction(trans2);
      trip.add_transaction(trans3);
